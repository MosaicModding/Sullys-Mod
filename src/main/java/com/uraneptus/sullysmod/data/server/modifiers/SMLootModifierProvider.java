...
import com.uraneptus.sullysmod.core.registry.loot.SMLootConditions;
...
public class SMLootModifierProvider extends LootModifierProvider {
    ...
    protected void registerEntries(HolderLookup.Provider provider) {
        this.entry("lanternfish_to_fishing").selects(BuiltInLootTables.FISHING_FISH).addModifier(basicEntriesModifier(createLootEntryWithCondition(SMItems.LANTERNFISH.get(), 55, 1, 1, SMLootConditions.ON_LANTERNFISH_HEIGHT, SMFeatureSelection.LANTERNFISH)));
        this.entry("piranha_to_fishing").selects(BuiltInLootTables.FISHING_FISH).addModifier(basicEntriesModifier(createLootEntryWithCondition(SMItems.PIRANHA.get(), 40, 1, 1, SMLootConditions.getPiranhaBiomes(), SMFeatureSelection.PIRANHA)));
        this.entry("music_disc_scour_to_jungle_temple").selects(BuiltInLootTables.JUNGLE_TEMPLE).addModifier(basicEntriesModifier(createMusicDiscLootEntry(SMItems.MUSIC_DISC_SCOUR.get(), 4, SMFeatureSelection.JADE)));
        ...
        this.entry("jade_smithing_template_to_jungle_temple").selects(BuiltInLootTables.JUNGLE_TEMPLE).addModifier(basicEntriesModifier(createLootEntry(SMItems.JADE_UPGRADE_SMITHING_TEMPLATE.get(), 4, 1, 2, SMFeatureSelection.JADE)));
        this.entry("throwing_knife_to_shipwreck_supply").selects(BuiltInLootTables.SHIPWRECK_SUPPLY).addModifier(basicEntriesModifier(createLootEntry(SMItems.THROWING_KNIFE.get(), 3, 2, 5, SMFeatureSelection.PIRANHA)));
        this.entry("throwing_knife_to_simple_dungeon").selects(BuiltInLootTables.SIMPLE_DUNGEON).addModifier(basicEntriesModifier(createLootEntry(SMItems.THROWING_KNIFE.get(), 15, 3, 7, SMFeatureSelection.PIRANHA)));
        this.entry("throwing_knife_to_jungle_temple_dispenser").selects(BuiltInLootTables.JUNGLE_TEMPLE_DISPENSER).addModifier(basicEntriesModifier(createLootEntry(SMItems.THROWING_KNIFE.get(), 10, 2, 7, SMFeatureSelection.PIRANHA)));
        this.entry("throwing_knife_to_underwater_ruin_big").selects(BuiltInLootTables.UNDERWATER_RUIN_BIG).addModifier(basicEntriesModifier(createLootEntry(SMItems.THROWING_KNIFE.get(), 5, 1, 6, SMFeatureSelection.PIRANHA)));
        this.entry("throwing_knife_to_underwater_ruin_small").selects(BuiltInLootTables.UNDERWATER_RUIN_SMALL).addModifier(basicEntriesModifier(createLootEntry(SMItems.THROWING_KNIFE.get(), 5, 1, 5, SMFeatureSelection.PIRANHA)));
        this.entry("throwing_knife_to_buried_treasure").selects(BuiltInLootTables.BURIED_TREASURE).addModifier(basicEntriesModifier(createLootEntry(SMItems.THROWING_KNIFE.get(), 2, 1, 7, SMFeatureSelection.PIRANHA)));
        this.entry("throwing_knife_to_desert_pyramid").selects(BuiltInLootTables.DESERT_PYRAMID).addModifier(basicEntriesModifier(createLootEntry(SMItems.THROWING_KNIFE.get(), 20, 1, 8, SMFeatureSelection.PIRANHA)));
    ...
    private LootPoolEntryContainer createLootEntryWithCondition(ItemLike item, int weight, int min, int max, LootItemCondition.Builder condition, SMFeatureSelection... features) {
    ...
    private LootPoolEntryContainer createLootEntry(ItemLike item, int weight, int min, int max, SMFeatureSelection... features) {
    ...
    private LootPoolEntryContainer createMusicDiscLootEntry(ItemLike item, int weight, SMFeatureSelection... features) {
