...
public class SMAdventureAdvancements implements ForgeAdvancementProvider.AdvancementGenerator {
    ...
    private static final List<Item> ARTIFACTS = SMArtifacts.ARTIFACT_DESC_MAP.stream().map(RegistryObject::get).collect(Collectors.toList());
    private static final List<Item> BIBLIOPHILE_ITEMS = List.of(SMArtifacts.TORN_MANUSCRIPT.get(), SMArtifacts.LOST_JOURNAL.get(), SMArtifacts.LOST_SKETCHBOOK.get(), SMArtifacts.LOST_RECIPE_BOOK.get());
    private static final List<Item> ALL_SKULL_ITEMS = SMArtifacts.ANCIENT_SKULLS.stream().map(Supplier::get).filter(block -> block != SMArtifacts.UNICORN_ANCIENT_SKULL.getFirst().get()).map(Block::asItem).collect(Collectors.toList()); //TODO maybe add some advancement trigger to add a check for the unicorn skull
    private static final List<Item> ALL_FOSSIL_ITEMS = List.of(SMArtifacts.FOSSILISED_BONE.get(), SMArtifacts.FOSSILISED_FISH.get(), SMArtifacts.FOSSILISED_FOOTSTEP.get(), SMArtifacts.FOSSILISED_SHELLS.get());
    ...
    public void generate(HolderLookup.Provider registries, Consumer<Advancement> saver, ExistingFileHelper existingFileHelper) {
        ...
        Advancement FIRST_ARTIFACT = createAdv(FIRST_ARTIFACT_ADV_NAME, null, FrameType.TASK, SMArtifacts.BROKEN_VASE.get(), FIRST_ARTIFACT_ADV, addCriterion("first_artifact", InventoryChangeTrigger.TriggerInstance.hasItems(ItemPredicate.Builder.item().of(SMItemTags.ARTIFACTS).build())), null, saver, existingFileHelper);
        Advancement LOST_CROWN = createAdv(LOST_CROWN_ADV_NAME, FIRST_ARTIFACT, FrameType.TASK, SMArtifacts.LOST_CROWN.get(), LOST_CROWN_ADV, addCriterion("lost_crown", InventoryChangeTrigger.TriggerInstance.hasItems(SMArtifacts.LOST_CROWN.get())), null, saver, existingFileHelper);
        Advancement BIBLIOPHILE = addCollectionAdv(BIBLIOPHILE_ITEMS, BIBLIOPHILE_ADV_NAME, createAdvBuilder(FIRST_ARTIFACT, FrameType.GOAL, SMArtifacts.TORN_MANUSCRIPT.get(), BIBLIOPHILE_ADV, null, null), saver, existingFileHelper);
        Advancement FIRST_SKULL = createAdv(FIRST_SKULL_ADV_NAME, FIRST_ARTIFACT, FrameType.TASK, SMArtifacts.CRESTED_ANCIENT_SKULL.getFirst().get(), FIRST_SKULL_ADV, addCriterion("first_skull", InventoryChangeTrigger.TriggerInstance.hasItems(ItemPredicate.Builder.item().of(SMItemTags.ANCIENT_SKULLS).build())), null, saver, existingFileHelper);
        Advancement ALL_SKULLS = addCollectionAdv(ALL_SKULL_ITEMS, ALL_SKULLS_ADV_NAME, createAdvBuilder(FIRST_SKULL, FrameType.CHALLENGE, SMArtifacts.CRESTED_ANCIENT_SKULL.getFirst().get(), ALL_SKULLS_ADV, null, AdvancementRewards.Builder.experience(100)), saver, existingFileHelper);
        Advancement DRIED_FLOWERS = addCollectionAdv(List.of(SMArtifacts.DRIED_CYAN_FLOWER.get().asItem(), SMArtifacts.DRIED_RED_FLOWER.get().asItem()), DRIED_FLOWERS_ADV_NAME, createAdvBuilder(FIRST_ARTIFACT, FrameType.GOAL, SMArtifacts.DRIED_CYAN_FLOWER.get(), DRIED_FLOWERS_ADV, null, null), saver, existingFileHelper);
        Advancement FIND_TABLETS = addCollectionAdv(List.of(SMArtifacts.MOON_TABLET.get(), SMArtifacts.OMINOUS_TABLET.get()), FIND_TABLETS_ADV_NAME, createAdvBuilder(FIRST_ARTIFACT, FrameType.GOAL, SMArtifacts.MOON_TABLET.get(), FIND_TABLETS_ADV, null, null), saver, existingFileHelper);
        Advancement ALL_FOSSILS = addCollectionAdv(ALL_FOSSIL_ITEMS, ALL_FOSSILS_ADV_NAME, createAdvBuilder(FIRST_ARTIFACT, FrameType.GOAL, SMArtifacts.FOSSILISED_BONE.get(), ALL_FOSSILS_ADV, null, null), saver, existingFileHelper);
        Advancement ALL_ARTIFACTS = addCollectionAdv(ARTIFACTS, ALL_ARTIFACTS_ADV_NAME, createAdvBuilder(FIRST_ARTIFACT, FrameType.CHALLENGE, SMArtifacts.BROKEN_VASE.get(), ALL_ARTIFACTS_ADV, null, AdvancementRewards.Builder.experience(700)), saver, existingFileHelper);
    }
