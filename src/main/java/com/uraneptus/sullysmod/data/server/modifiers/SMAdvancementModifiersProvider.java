...
public class SMAdvancementModifiersProvider extends AdvancementModifierProvider {
    ...
    protected void registerEntries(HolderLookup.Provider provider) {
        ...
        this.entry("husbandry/tactical_fishing").selects("husbandry/tactical_fishing")
                .addModifier(CriteriaModifier.builder(this.modId).addCriterion("lanternfish_bucket", FilledBucketTrigger.TriggerInstance.filledBucket(ItemPredicate.Builder.item().of(SMItems.LANTERNFISH_BUCKET.get()).build())).addIndexedRequirements(0, false, "lanternfish_bucket").build())
                .addModifier(CriteriaModifier.builder(this.modId).addCriterion("piranha_bucket", FilledBucketTrigger.TriggerInstance.filledBucket(ItemPredicate.Builder.item().of(SMItems.PIRANHA_BUCKET.get()).build())).addIndexedRequirements(0, false, "piranha_bucket").build());
