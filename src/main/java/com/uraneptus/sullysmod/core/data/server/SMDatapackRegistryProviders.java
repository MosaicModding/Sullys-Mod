...
public class SMDatapackRegistryProviders {
    ...
    public static void registerDatapackProviders(ExistingFileHelper fileHelper, DataGenerator generator, RegistryOps<JsonElement> registryOps){
        /**
         * This should probably be cleaned up at some point, but it does the job for now.
        */
        ...
        final ConfiguredFeature<OreConfiguration, ?> JADE_ORE_CONFIGURED = new ConfiguredFeature<>(Feature.ORE, new OreConfiguration(JADE_ORE_TARGET_LIST, 10, 1.0F));
        ...
        //Biome Tags
        final HolderSet.Named<Biome> JADE_BIOMES = new HolderSet.Named<>(registryOps.registry(Registry.BIOME_REGISTRY).get(), SMBiomeTags.JADE_GENERATES_IN);
        final HolderSet.Named<Biome> TORTOISE_BIOMES = new HolderSet.Named<>(registryOps.registry(Registry.BIOME_REGISTRY).get(), SMBiomeTags.TORTOISES_SPAWN_IN);
        final HolderSet.Named<Biome> LANTERNFISH_BIOMES = new HolderSet.Named<>(registryOps.registry(Registry.BIOME_REGISTRY).get(), SMBiomeTags.LANTERNFISH_SPAWN_IN);

        //Biome Modifiers
        final BiomeModifier JADE_MODIFIER = new ForgeBiomeModifiers.AddFeaturesBiomeModifier(JADE_BIOMES,
                HolderSet.direct(registryOps.registry(Registry.PLACED_FEATURE_REGISTRY).get().getOrCreateHolderOrThrow(ResourceKey.create(Registry.PLACED_FEATURE_REGISTRY, JADE_ORE_RL))),
